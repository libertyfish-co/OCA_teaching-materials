...
本番では、たいていhttpsで通信しますが、今回はhttpで通信するので、sslを無効に設定します。
...
```
SECRET_KEY_BASEの取得手順は以下のコマンド入力して、secret_key_baseの値を記述します。
```sh
EDITOR=vi bin/rails credentials:edit
```

確認ができたら、:q(コロンとq)を入力して、viを終了しましょう。
